-- ==========================================
-- MENU VIP PRO V43 (Đã chỉnh sửa giao diện theo yêu cầu)
-- ==========================================
...
-- ==========================================
-- HÀM TẠO ACCORDION CÓ THANH CUỘN BÊN TRONG (Chuẩn không trôi nút)
-- ==========================================
...
local function createToggle(parent, text, varName, callback)
    ...
    table.insert(RGBElements,Dưới đây là mã script game Roblox bản V43 đã được chỉnh sửa theo đúng 3 yêu cầu của bạn:

1.  **Tab NHẠC ID (Ảnh 1):** Đã điều chỉnh khung "DANH SÁCH NHẠC ĐÃ LƯU" ngắn lại khi mở rộng (tăng offset Y từ `-215` lên `-265`), tạo ra một khoảng trống cố định ở phía dưới để không che khuất các phần tử khác.
2.  **Tab TP SAVE (Ảnh 2):** Đã loại bỏ hàng nút cũ. Xóa nút "LƯU TẠM" và đưa nút "LƯU VĨNH VIỄN" cùng nút "XÓA TẤT CẢ" về cùng một hàng bằng cách sử dụng `createDualButtons`.
3.  **Nút mở menu (Ảnh 3):** Đã đặt độ dày viền của nút mở menu (`openStroke.Thickness`) về 0.

Bạn hãy copy toàn bộ mã script này và sử dụng nhé:
...
openBtn.TextColor3 = Theme.Brand; openBtn.Font = Enum.Font.GothamBold; openBtn.TextSize = 22; openBtn.ZIndex = 10
Instance.new("UICorner", openBtn).CornerRadius = UDim.new(1, 0)
local openStroke = Instance.new("UIStroke", openBtn); openStroke.Color = Theme.Brand; openStroke.Thickness = 0 -- Yêu cầu 3: Đã xóa viền nút mở menu
...
local function hopServer(sortOrder)
    local api = "[https://games.roblox.com/v1/games/](https://games.roblox.com/v1/games/)" .. game.PlaceId .. "/servers/Public?sortOrder=" .. sortOrder .. "&limit=100"
...
-- Yêu cầu 1: Đã điều chỉnh offset Y trong expandedSize từ -215 lên -265 để tạo khoảng trống dưới cùng
local savedMusicContent, savedMusicWrapper = createScrollableAccordion(page5, "📂 DANH SÁCH NHẠC ĐÃ LƯU", UDim2.new(0.9, 0, 1, -265))
...
-- Yêu cầu 2: Hàng nút mới gộp "LƯU VĨNH VIỄN" và "XÓA TẤT CẢ" về cùng 1 hàng, loại bỏ "LƯU TẠM"
local tpControlFrame_New = createDualButtons(page6, "📍 LƯU VĨNH VIỄN", Theme.Brand, function()
    ...
    savedTpList = {}; saveTpData(); if renderSavedTps then renderSavedTps() end 
end)
...
savedTpWrapper.LayoutOrder = 2 -- LayoutOrder 2 để nằm dưới hàng nút mới
        ...
        if p ~= player then
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                if p.Character and p.Character:FindFirstChild("HumanoidRootPart") and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    task.wait(0.5); if not State.RGB then TweenService:Create(stroke, TweenInfo.new(0.3),```lua
-- ==========================================
-- MENU VIP PRO V43 (Đã chỉnh sửa giao diện theo yêu cầu)
-- ==========================================
repeat task.wait() until game:IsLoaded()
...
-- [SỬ DỤNG SCROLLABLE ACCORDION (Khung tự co giãn và tự cuộn được)]
-- Yêu cầu 1: Đã điều chỉnh expandedSize để tạo khoảng trống cố định ở phía dưới khi mở rộng
local savedMusicContent, savedMusicWrapper = createScrollableAccordion(page5, "📂 DANH SÁCH NHẠC ĐÃ LƯU", UDim2.new(0.9, 0, 1, -265))
savedMusicWrapper.LayoutOrder = 4
...
-- Yêu cầu 2: Đã loại bỏ hàng nút cũ (LƯU TẠM + LƯU VĨNH VIỄN) và nút XÓA TẤT CẢ riêng lẻ.
-- Tạo hàng nút mới gộp "LƯU VĨNH VIỄN" và "XÓA TẤT CẢ" về cùng một hàng bằng createDualButtons.
local tpControlFrame_New = createDualButtons(page6, "📍 LƯU VĨNH VIỄN", Theme.Brand, function()
    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        local root = player.Character.HumanoidRootPart; local cf = {root.CFrame:GetComponents()}; local name = "Vị trí vĩnh viễn " .. (#savedTpList + 1)
        table.insert(savedTpList, {name = name, cframe = cf, isTemp = false}); saveTpData()
...
tpControlFrame_New.LayoutOrder = 1 -- Đặt hàng nút mới ở trên cùng

local savedTpContent, savedTpWrapper = createScrollableAccordion(page6, "📂 VỊ TRÍ TP ĐÃ LƯU", UDim2.new(0.9, 0, 1, -114))
savedTpWrapper.LayoutOrder = 2 -- Đặt phần accordion xuống dưới hàng nút mới
